...
def generate_cursor_trajectories(
    ...
    # Create dummy anchor (will be generated by model if needed)
    # For generation, we use a simple anchor at the normalized endpoint
    pred_len = hyper_params.pred_len
    anchor = torch.zeros(1, pred_len, 2, device=device)
    # Linear interpolation from current position to end
    for i in range(pred_len):
        t = (i + 1) / pred_len
        anchor[0, i, 0] = t  # X goes from 0 to 1
        anchor[0, i, 1] = 0  # Y stays at 0
...
def main():
    ...
    print(f"Distance: {np.linalg.norm(np.array(end) - np.array(start)):.1f} pixels")
    print(f"Distance group: {get_distance_group(np.linalg.norm(np.array(end) - np.array(start)))}")
